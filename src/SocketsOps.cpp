...
int sockets::createNonblockingOrDie(sa_family_t family)
{
  ...
  if (sockfd < 0)
  {
    LOG_INFO("sockets::createNonblockingOrDie");
  }
#endif
  return sockfd;
}
...
void sockets::shutdownWrite(int sockfd)
{
  ...
  {
    LOG_INFO("sockets::shutdownWrite");
  }
}
                       ...
                       const struct sockaddr *addr)
{
  if (addr->sa_family == AF_INET6)
  {
    ...
    const struct sockaddr_in *addr6 = sockaddr_in_cast(addr);
    uint16_t port = sockets::networkToHost16(addr6->sin_port);
    assert(size > end);
                   ...
                   const struct sockaddr *addr)
{
  ...
  else if (addr->sa_family == AF_INET6)
  {
    ...
    const struct sockaddr_in *addr6 = sockaddr_in_cast(addr);
    ::inet_ntop(AF_INET6, &addr6->sin_addr, buf, static_cast<socklen_t>(size));
  }
}
